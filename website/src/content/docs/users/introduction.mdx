...
| --- | --- | :---: |
| Start every service in the `media` group | `docker compose up -d jellyfin sonarr radarr …` | `s` |
...
| Undo a bad edit | `git diff && git checkout compose.yml` *(in a repo)* | `4`, then `r` |
...
Every write is snapshotted into `.cais/backups/` first. Tab `4` lists every stored copy of the compose file and `.env`, newest first, with a live preview. `r` restores; the current file is snapshotted first, **so a restore is itself undoable**.
...
- **Fourteen themes**, previewed live as you move the cursor.
