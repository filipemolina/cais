...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		// catalog rather than closing the overlay outright.
		...
		// description fuzzy-matches. Reuses List.Filter's binding ("/") so
		// the affordance matches the one every list in the app already uses.
		case key.Matches(msg, keys.List.Filter):
			m.filterTyping = true
			m.filterInput.Focus()
			return m, nil
		...
		// The catalog is longer than a terminal, so it scrolls.
